package com.toolbox.adapters.headerFooterAdapter;
...
/**
 * Created by Will on 2/8/2015. Based On https://github.com/willblaschko/header-footer-recycler-view
 */
public class RecyclerViewHeaderFooterAdapter<T, U extends RecyclerViewLogic<T>> extends RecyclerView.Adapter {
    ...
    @IntDef({TYPE_MANAGER_OTHER, TYPE_MANAGER_LINEAR, TYPE_MANAGER_GRID, TYPE_MANAGER_STAGGERED_GRID})
    ...
    private U mIntermediary;


    public RecyclerViewHeaderFooterAdapter(RecyclerView.LayoutManager manager, U intermediary) {
        setManager(manager);
        this.mIntermediary = intermediary;
        this.mIntermediary.registerAdapterDataObserver(new InternalAdapterDataObserver());
    }

    public U getInternalAdapter() {
    ...
    public int getGridSpan(int position) {
        ...
        T intermediaryItem = mIntermediary.getItem(position);
        if (intermediaryItem instanceof FullWidthItem) {
            return getSpan();
        } else if (intermediaryItem instanceof GridItem) {
            return ((GridItem) intermediaryItem).getGridSpan();
        }
        return 1;
    }
    ...
    public int getItemCountWithoutHeadersAndFooters() {
        return mIntermediary.getItemCount();
    }
    ...
    public int getItemViewType(int position) {
        ...
        int type = mIntermediary.getItemViewType(position - mHeaders.size());
        if (type == TYPE_HEADER || type == TYPE_FOOTER) {
    ...
    public void addFooterOnTop(View footer) {
        if (!mFooters.contains(footer)) {
            mFooters.add(0, footer);
            //animate
            notifyItemInserted(mHeaders.size() + mIntermediary.getItemCount());
        }
    }
    ...
    public void add(T item, int position) {
        mIntermediary.addItem(item, position);
    }

    public void addItems(Collection<? extends T> items, boolean initial) {
        mIntermediary.addItems(items, initial);
    }

    private int getIntermediaryPosition(int positionFromInternalAdapter) {
        return mHeaders.size() + positionFromInternalAdapter;
    }
    ...
    private class InternalAdapterDataObserver extends RecyclerView.AdapterDataObserver {
        ...
        public void onItemRangeChanged(int positionStart, int itemCount) {
            notifyItemRangeChanged(getIntermediaryPosition(positionStart), itemCount);
        }
        ...
        public void onItemRangeInserted(int positionStart, int itemCount) {
            notifyItemRangeInserted(getIntermediaryPosition(positionStart), itemCount);
        }

        @Override
        public void onItemRangeRemoved(int positionStart, int itemCount) {
            notifyItemRangeRemoved(getIntermediaryPosition(positionStart), itemCount);
        }

        @Override
        public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
            notifyItemMoved(getIntermediaryPosition(fromPosition), getIntermediaryPosition(toPosition));
        }
    }
}
